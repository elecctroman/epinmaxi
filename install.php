    ...
    case 2:
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            try {
                ...
                echo '<p class="card" style="max-width:520px;">Veritabanı bağlantısı kurulamadı: ' . htmlspecialchars($e->getMessage()) . '</p>';
                echo '<p><a class="button-primary" href="?step=2">Geri dön</a></p>';
                render_footer();
                break;
            }

            $configString = "<?php\nreturn " . var_export($db, true) . ";\n";
            if (file_put_contents(__DIR__ . '/config/database.php', $configString) === false) {
                render_header('Adım 2 - Veritabanı Bilgileri');
                echo '<p class="card" style="max-width:520px;">config/database.php dosyası yazılamadı. Dosya izinlerini kontrol edin.</p>';
                echo '<p><a class="button-primary" href="?step=2">Geri dön</a></p>';
                render_footer();
                break;
            }
    ...
    case 3:
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            try {
                ...
                run_sql_file($pdo, __DIR__ . '/sql/database.sql');
                ...
                render_header('Adım 3 - Veritabanı Kurulumu');
                echo '<p>Kurulum sırasında hata oluştu: ' . htmlspecialchars($e->getMessage()) . '</p>';
                echo '<a class="button-primary" href="?step=3">Tekrar dene</a>';
                render_footer();
                exit;
            }
        }
    ...
    case 4:
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            try {
                ...
                echo '<p class="card" style="max-width:520px;">Veritabanı bağlantısı başarısız: ' . htmlspecialchars($e->getMessage()) . '</p>';
                echo '<p><a class="button-primary" href="?step=4">Tekrar dene</a></p>';
                render_footer();
                break;
            }
