...
class CatalogController extends Controller
{
    ...
    {
        $category = DB::query('SELECT * FROM categories WHERE slug = :slug LIMIT 1', ['slug' => $slug])->fetch(PDO::FETCH_ASSOC);
        if (!$category) {
    ...
    {
        $product = DB::query('SELECT p.*, c.name AS category_name, c.slug AS category_slug FROM products p LEFT JOIN categories c ON c.id = p.category_id WHERE p.slug = :slug AND p.status = "active" LIMIT 1', ['slug' => $slug])->fetch(PDO::FETCH_ASSOC);
        if (!$product) {
            http_response_code(404);
