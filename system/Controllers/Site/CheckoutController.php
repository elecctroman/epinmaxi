...
use System\Services\NotificationService;
...
class CheckoutController extends Controller
{
    ...
    {
        try {
            if (!rate_limit('checkout:' . ($_SERVER['REMOTE_ADDR'] ?? 'cli'), 5, 120)) {
                throw new \RuntimeException('Çok fazla ödeme denemesi. Lütfen birkaç dakika sonra tekrar deneyin.');
            }
            $summary = $this->cart->totals();
            ...
            $orderUrl = rtrim(setting('app.url', $scheme . '://' . $host), '/') . '/siparis/' . $orderNo;
            $notifier->orderCreated($customerEmail, [
                'order_no' => $orderNo,
                'total' => $grand,
                'currency' => $summary['currency'],
                'customer' => $customer,
            ...
            if ($order['status'] === 'success') {
                $notifier->orderPaid($customerEmail, [
                    'order_no' => $orderNo,
                    'payment_method' => $gatewayName,
                    'total' => $grand,
                    'currency' => $summary['currency'],
                ...
                $notifier->orderDelivered($customerEmail, [
                    'order_no' => $orderNo,
                    'order_url' => $orderUrl,
                    'customer' => $customer,
                ]);
                if (!empty($customer['phone'])) {
                    $notifier->sms($customer['phone'], 'Siparisiniz hazir: ' . $orderNo);
                }
