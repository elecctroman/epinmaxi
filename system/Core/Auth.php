...
class Auth
{
    ...
    public static function attempt(string $email, string $password): bool
    {
        ...
        if ($user && ($user['status'] ?? 'active') !== 'active') {
            if (($user['status'] ?? '') === 'pending') {
                throw new \RuntimeException('Hesabınız henüz doğrulanmadı. Lütfen e-posta adresinizi doğrulayın.');
            }
    ...
    public static function register(string $name, string $email, string $password, ?string $phone = null, bool $autoLogin = false): array
    {
        self::ensureSession();
        $hash = password_hash($password, PASSWORD_DEFAULT);
        DB::query('INSERT INTO users (role, name, email, phone, password_hash, status, created_at) VALUES ("customer", :name, :email, :phone, :hash, :status, NOW())', [
            'name' => $name,
            'email' => $email,
            'phone' => $phone,
            'hash' => $hash,
            'status' => $autoLogin ? 'active' : 'pending',
        ]);
