...
4. Kurulum tamamlandığında ekranda görüntülenen API token'ınızı güvenle saklayın.
...
## .htaccess
`public/.htaccess` dosyası pretty URL yönlendirmesi, gzip/deflate ve güvenlik başlıklarını hazırlar. Apache kullanıyorsanız etkinleştirin.
...
- Yedekleme: `sql/schema.sql` + `mysqldump` ile günlük tam yedek, `storage/uploads` ve `storage/logs` dosya sistemi kopyası.
