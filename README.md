...
2. `config/database.ini.example` dosyasını `config/database.ini` olarak kopyalayın ve **yalnızca** `key=value` satırlarını düzenleyerek veritabanı bilgilerinizi girin (tırnak veya noktalı virgül kullanmayın). Gelişmiş senaryolar için `config/database.local.php` dosyası oluşturabilirsiniz. Çekirdek `config/database.php` dosyasını düzenlemeyin.
...
- Ortam değişkeni kullanılmaz; veritabanı erişim bilgileri `config/database.local.php` veya `config/database.ini` üzerinden yönetilir ve çekirdek dosya varsayılanları korur.
...
## SSS
- **Kurulum tekrar nasıl çalıştırılır?** Yeni bir veritabanı oluşturup `sql/database.sql` dosyasını yeniden içe aktarın; gerekirse `config/database.ini` (veya `config/database.local.php`) dosyasındaki bağlantı bilgilerini güncelleyin.
